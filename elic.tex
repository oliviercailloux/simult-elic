...
Assume we are able to obtain a strict partial order $\psprefsv \subseteq \sprefsv$. Define our approximation of $G$, $G_{\psprefsv}$, as the rule that selects the maximal elements according to $\psprefsv$, given a complete profile: $x$ wins in $G_{\psprefsv}(\profshort)$ iff there is no $y$ in $\profshort$ such that $y \psprefsv x$. The rule $G_{\psprefsv}$ gives us information about the real rule, $G$, in the sense that any real winner is in the winners selected by $G_{\psprefsv}$.
\begin{fact}
	Given $\psprefsv \subseteq \sprefsv$, $\forall \profshort: G(\profshort) \subseteq G_{\psprefsv}(\profshort)$.
\end{fact}
\begin{proof}
	Consider $\profshort$ and assume $y$ does not win in $G_{\psprefsv}(\profshort)$. We show that $y$ does not win in $G(\profshort)$. We know that $y$ is dominated by some $x$ in $\profshort$, thus $x \psprefsv y$ and both $x$ and $y$ are in $\profshort$. Because $x \psprefsv y$, $x \sprefsv y$, thus $y$ does not win in $G(\profshort)$.
\end{proof}

Assume we know $R \subseteq \sprefsv$ and $S \subseteq \iprefsv$ (given directly as answers to our queries). Then we can define $\pprefsv = (R ∪ S)^T$ and obtain $\pprefsv \subseteq \prefsv$. Defining $\psprefsv$ and $\piprefsv$ as the asymmetric and symmetric parts of $\pprefsv$, we obtain that $\psprefsv \subseteq \sprefsv$.
\begin{proof}
	(Only the second part.) Given $x \pprefsv y$, we show that if $x \iprefsv y$ then $x S^T y$ thus $y \pprefsv x$, which shows that $x \psprefsv y ⇒ ¬(x \iprefsv y)$, which shows that when $x \psprefsv y$, as $x \pprefsv y$, $x \prefsv y$, and as $¬(x \iprefsv y)$, $x \sprefsv y$.
	
	The main point holds because, if $x \pprefsv y$ and $x \iprefsv y$, from $x$ we can reach $y$ through $R ∪ S$, and thus from $x$ we can reach $y$ using $S$ only (because if we need to use $R$, then $x R w [R ∪ S]^T y$, or something similar, and then $x \sprefsv y$).
\end{proof}
